...
\begin{figure*}[ht!]
\centering
\includegraphics[width=0.95\textwidth]{../../results/figures/fig_v17.3_killer_plot_suite.pdf}
...
\begin{figure*}[ht!]
\centering
\includegraphics[width=0.95\textwidth]{../../results/figures/fig_v17.3_clustering_analysis.pdf}
...
\begin{figure*}[ht!]
\centering
\includegraphics[width=0.95\textwidth]{../../results/figures/fig_v17.3_metallicity_evolution.pdf}
...
\subsection{NEW v17.2: Bootstrap Validation}\label{sec:bootstrap}

To ensure statistical robustness, we perform bootstrap resampling (1000 iterations) of the galaxy catalog:

\begin{figure*}[ht!]
\centering
\includegraphics[width=0.95\textwidth]{../../results/figures/fig_v17.3_bootstrap_distributions.pdf}
\caption{\textbf{Bootstrap Distributions of $\Delta\chi^2$ and $\Delta$BIC (v17.2).}
...
\subsection{NEW v17.2: Epsilon Sensitivity Analysis}

We systematically explore $\chi^2$ as a function of fixed star formation efficiency $\varepsilon \in [0.05, 0.20]$:

\begin{figure}[ht!]
\centering
\includegraphics[width=0.95\columnwidth]{../../results/figures/fig_v17.3_epsilon_sensitivity.pdf}
\caption{\textbf{$\chi^2$ Sensitivity to $\varepsilon$ (v17.2).}
...
\subsection{NEW v17.3: Full MCMC Analysis with Convergence Diagnostics}\label{sec:mcmc}
...
\textbf{New v17.3 Figures}:
\begin{itemize}
    \item \textbf{fig\_v17.3\_mcmc\_trace.pdf}: Trace plots showing walker evolution over 100,000 steps, verifying chain mixing and stationarity
    \item \textbf{fig\_v17.3\_mcmc\_autocorr.pdf}: Autocorrelation function decay for JANUS and $\Lambda$CDM, demonstrating convergence
    \item \textbf{fig\_v17.3\_convergence\_diagnostics.pdf}: Summary table with $\tau$, $n_{\rm eff}$, acceptance rate, and convergence status
\end{itemize}
...
\begin{table*}[ht!]
\centering
\caption{JANUS vs. \LCDM: Paradigm Comparison (v17.3b)}
...
\textbf{v17.3 MCMC Analysis}:
This version implements full MCMC posterior sampling with $10^5$ steps per model:
\begin{itemize}
    \item 100,000 iterations with 32 walkers (emcee ensemble sampler)
    \item 20\% burn-in removal and autocorrelation-based thinning
    \item Convergence diagnostics: autocorrelation time $\tau$, effective samples $n_{\rm eff}$, acceptance rate
    \item Trace plots verify chain mixing and stationarity
    \item Robust 68\% and 95\% credible intervals on $\varepsilon$ posteriors
\end{itemize}
...
\begin{itemize}
    ...
    \item Quantitative [CII] luminosity function test for dusty galaxies
\end{itemize}
...
The v17 extension with 236 galaxies (including extreme dusty/metal-poor outliers) demonstrates the importance of multi-probe validation. We encourage the community to critically test both $\Lambda$CDM and JANUS predictions with upcoming JWST Cycle 3-4 data.
