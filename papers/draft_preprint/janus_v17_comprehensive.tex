...
\\
\small \textit{Data availability}: Extended galaxy catalog (200 sources with z, M$_*$, metallicity, dusty flags),\\
...
\begin{enumerate}
    \item \textbf{Extended high-$z$ sample}: 200 galaxies at $6.63 < z < 14.32$ from JADES Data Release 4 \citep{bunker2025,eisenstein2025}, EXCELS survey \citep{carnall2025,cullen2025}, GLASS \citep{morishita2025}, A3COSMOS blind mm catalog, CEERS \citep{finkelstein2024}, and UNCOVER \citep{bezanson2024}.
...
\section{Data: Extended JWST Catalog v17}\label{sec:data}

\subsection{Sample Compilation}

Our extended catalog (v17) combines spectroscopic and photometric redshifts from seven independent JWST and ALMA surveys:
...
The final catalog contains 200 galaxies at $6.63 < z < 14.32$ with the following properties:

\begin{itemize}
    \item \textbf{Redshifts}: 55 spectroscopic (27.5\%), 145 photometric (72.5\%)
    \item \textbf{Stellar masses}: $\log(M_*/M_\odot) = 8.45 - 10.57$ (extended to higher masses via dusty galaxies)
    \item \textbf{Metallicities}: 55 galaxies with $T_e$-based O/H measurements, including record low $12+\log({\rm O/H}) = 6.8$ at $z=12.15$
    ...
    \item \textbf{Dusty/NIRCam-dark}: 4 galaxies (AC-2168 + 3 A3COSMOS candidates) with dust attenuation $A_V > 3$ mag
\end{itemize}

\textbf{Key additions in v17}:
\begin{itemize}
    \item 50 new galaxies (150 $\to$ 200) extending mass range to $\log M_* = 10.6$
    \item First dusty/obscured sample testing JANUS predictions for hidden star formation
    \item 7 newly confirmed GLASS $z=9-11$ galaxies strengthening proto-cluster constraints
    \item Record-breaking metallicity measurement pushing chemical evolution tests to extremes
\end{itemize}
...
\begin{figure*}[ht!]
\centering
\includegraphics[width=0.95\textwidth]{../../results/figures/fig_v17_killer_plot_suite.pdf}
\caption{\textbf{Killer Plot Suite: Controlled Comparison at Fixed Astrophysics (v17 with 200 galaxies).}
...
\begin{figure*}[ht!]
\centering
\includegraphics[width=0.95\textwidth]{../../results/figures/fig_v17_clustering_analysis.pdf}
\caption{\textbf{Proto-Cluster Dynamics at $z \sim 7-10$ (v17).}
...
\textbf{Right:} Velocity dispersions ($\sigma_v \sim 165-198$ km/s) indicate dynamically relaxed systems, requiring rapid assembly. GHZ9-cluster (purple, 7 confirmed members) and A2744-z7p9 (orange, 7 members) show highest masses.
...
\textbf{Key Findings}:
\begin{itemize}
    \item Four proto-clusters: GHZ9-cluster (7 members, $z_{\rm mean}=10.14$), A2744-z13 (1 member, $z=12.63$), JD1-cluster (2 members, $z_{\rm mean}=10.32$), A2744-z7p9 (7 members, $z_{\rm mean}=7.89$)
    \item Mean velocity dispersions: $\langle \sigma_v \rangle = 177$ km/s (range: 165-198 km/s)
    \item Virial masses: $\log(M_{\rm vir}/M_\odot) = 19.86 - 20.14$ (mean $10^{20.0} M_\odot$)
    ...
    \item \textbf{GHZ9-cluster}: 7 spectroscopically confirmed members at $z=9.52-10.66$ (most robust high-$z$ proto-cluster) with AGN host GHZ9 at $z=10.145$
\end{itemize}
...
\begin{figure*}[ht!]
\centering
\includegraphics[width=0.95\textwidth]{../../results/figures/fig_v17_metallicity_evolution.pdf}
\caption{\textbf{Chemical Enrichment at High Redshift (v17 with "impossible" galaxy).}
...
\textbf{Key Findings}:
\begin{itemize}
    \item 55 galaxies with robust $T_e$-based O/H measurements spanning $6.8 < 12+\log({\rm O/H}) < 8.3$
...
The extended v17 sample includes 4 dusty/NIRCam-dark galaxies with $A_V > 3$ mag, providing a novel test of JANUS structure formation:
...
\begin{itemize}
    ...
    \item \textbf{A3COSMOS sample}: 3 additional NIRCam-dark candidates at $z \sim 6.9-7.2$ with $\log M_* \sim 10.3-10.4$ and SFR$>100$ \Msun/yr
\end{itemize}
...
We have presented the most comprehensive validation to date of JANUS bimetric cosmology using extended JWST January 2026 data (200 galaxies). Our key findings:
...
\begin{enumerate}
    ...
    \item \textbf{Proto-Cluster Dynamics}: Four confirmed proto-clusters at $z \sim 7-10$ (including GHZ9-cluster with 7 spectroscopic members) exhibit velocity dispersions ($\sigma_v \sim 177$ km/s) and virial masses ($M_{\rm vir} \sim 10^{20} M_\odot$) consistent with JANUS-enhanced clustering but challenging for $\Lambda$CDM hierarchical assembly.
...
\begin{itemize}
    ...
    \item \textbf{A3COSMOS}: \href{https://sites.google.com/view/a3cosmos/data}{https://sites.google.com/view/a3cosmos/data}
    \item \textbf{Extended catalog v17}: Available upon request to pg@gfo.bzh or via GitHub repository (JANUS-Z)
\end{itemize}
