...
\subsection{NEW v17.2: Bootstrap Validation}
...
\subsection{NEW v17.4: [CII] 158$\mu$m Luminosity Function Test}

We perform an \textbf{orthogonal validation} using the [CII] 158$\mu$m luminosity function (LF) from dusty/NIRCam-dark galaxies selected via blind ALMA continuum. This test is independent of UV-selected stellar mass functions.
...
\begin{itemize}
    ...
    \item [CII] luminosities: $\log(L_{[\rm CII]}/L_\odot) = 9.1 - 10.9$ (derived via De Looze+2014 calibration)
\end{itemize}

\textbf{[CII]-SFR Relation} (De Looze et al. 2014):
\begin{equation}
...
\end{equation}
\begin{itemize}
    \item $\Lambda$CDM: $L_* = 10^9 L_\odot$, $\alpha = -1.8$ (from Yan+2020, Loiacono+2021)
...
\textbf{Interpretation}: The small sample size ($N=24$) and concentration at the bright-end of the LF ($\log L > 9$) limit statistical discriminating power. Both models can accommodate the observed bright-end counts. Future ALMA surveys (REBELS DR2, $N > 100$) will enable definitive tests.
